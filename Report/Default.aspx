...
<asp:Content ID="Content2" ContentPlaceHolderID="cphRightCol" runat="server">
    ...
    <asp:SqlDataSource ID="sdsToDoItems" runat="server" ConnectionString="<%$ ConnectionStrings:RolePlayersGuild.Properties.Settings.rpgDBConn %>" DeleteCommand="DELETE FROM [ToDo_Items] WHERE [ItemID] = @ItemID" InsertCommand="INSERT INTO [ToDo_Items] ([ItemName], [ItemDescription], [StatusID], [TypeID], [AssignedToUserID], [CreatedByUserID]) VALUES (@ItemName, @ItemDescription, 1, 4, 0, @CreatedByUserID)" SelectCommand="SELECT ItemID, AssignedToUserID, AssignedToCharacterID, StatusID, ItemDescription, CharacterAssignable, DateReceived FROM [ToDo_Items]" UpdateCommand="UPDATE [ToDo_Items] SET [ItemName] = @ItemName, [ItemDescription] = @ItemDescription, [StatusID] = @StatusID, [TypeID] = @TypeID, [AssignedToUserID] = @AssignedToUserID, [CreatedByUserID] = @CreatedByUserID, [ThreadID] = @ThreadID WHERE [ItemID] = @ItemID">
        <InsertParameters>
